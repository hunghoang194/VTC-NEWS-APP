...
public protocol PlayerLayerViewDelegate : class {
...
open class PlayerLayerView: UIView {
    open weak var delegate: PlayerLayerViewDelegate?
    ...
    open func share() {
        NotificationCenter.default
        .post(name: NSNotification.Name("shareLink"),
         object: nil)
    }
            ...
            if item == self.playerItem {
                ...
                case "status":
                    if item.status == .failed || player?.status == AVPlayer.Status.failed {
                        ...
                        self.state = .buffering
                        if shouldSeekTo != 0 {
                            print("PlayerLayer | Should seek to \(shouldSeekTo)")
