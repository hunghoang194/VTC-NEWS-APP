    ...
    public init(url: URL, encoding: String.Encoding? = nil) {
        DispatchQueue.global(qos: .background).async {[weak self] in
            do {
                ...
                self?.groups = PlayerSubtitles.parseSubRip(string) ?? []
            } catch {
                print("| Player | [Error] failed to load \(url.absoluteString) \(error.localizedDescription)")
            }
        }
    }
