...
import AVKit
import GoSwiftyM3U8
open class VTVCabResourceLoaderDelegateSecure : NSObject{
    fileprivate static let instance = VTVCabResourceLoaderDelegateSecure()
    ...
    public  static var shared: VTVCabResourceLoaderDelegateSecure {
        get {
            return instance
        }
    }
    ...
    open func playerItemM3u8(with url: String) -> AVPlayerItem {
        self.streamUrl = url
        let url1 = NSURL(string: url)
        let mimeType = "audio/aac; codecs=\"mp4a.40.2\""
        let urlAsset = AVURLAsset(url: url1! as URL, options: ["AVURLAssetOutOfBandMIMETypeKey": mimeType])
        self.host = URL(string: url)?.absoluteURL.host
        urlAsset.resourceLoader.setDelegate(self, queue: queue)
        urlAsset.resourceLoader.preloadsEligibleContentKeys = true
        let item = AVPlayerItem(asset: urlAsset)
    ...
    open func handleRedirect(url : String, _ loadingRequest : AVAssetResourceLoadingRequest)->Bool{
        ...
        let newUrl = url.replacingOccurrences(of: "sdk", with: "https")
    ...
    open func handleManifest(_ loadingRequest : AVAssetResourceLoadingRequest){
        DispatchQueue.main.async {
            ...
            let playlist = try? String.init(contentsOf: URL(string: self.convertScheme(url: loadingRequest.request.url!, old: "sdk", new: "https"))!)
            if let playlist = playlist{
                let params = M3U8Parser.Params(playlist: playlist, playlistType: .master, baseUrl: URL(string: "https://"+self.host!)!)
                var original = ""
                do {
                            ...
                            if(line.contains("EXT-X-KEY:")){
                                ...
                                for component in furtherComponents {
                                    ...
                                    if component.contains("URI") {
                                        ...
                                        let a = "###"
                                        let replaceString = "ckey://61.28.235.91:8443/hlsdrm-service/api/getkey?kid="
    ...
    open func getActualURL(url: URL) -> String {
        ...
        return url.absoluteString.replacingOccurrences(of: "ckey", with: "https")
    }
...
extension VTVCabResourceLoaderDelegateSecure: URLSessionDelegate {
...
extension VTVCabResourceLoaderDelegateSecure : AVAssetResourceLoaderDelegate{
